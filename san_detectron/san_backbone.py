...
class SAN(Backbone):
    ...
    def forward(self, x):
        ...
        x = self.relu(self.bn0(self.layer0(self.conv0(self.pool(x))))) #64
        x = self.relu(self.bn1(self.layer1(self.conv1(self.pool(x))))) #256
        outputs['res2'] = x
        x = self.relu(self.bn2(self.layer2(self.conv2(self.pool(x))))) #512
        outputs['res3'] = x
        x = self.relu(self.bn3(self.layer3(self.conv3(self.pool(x))))) #1024
        outputs['res4'] = x
